...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const FindX = x2 - x1;
  const FindY = y2 - y1;
  const res = Math.sqrt(FindX ** 2 + FindY ** 2);
  return res;
}
...
// Еще одно решение, но тестам не нравится рекурсия и ругается на то, что возвращается функция
// function isPowerOfTwo(n) {
// 	if (n / 2 > 1) {
...
function isNumber(n) {
  return (
    (typeof n === 'number' || (typeof n !== 'string' && !Number.isNaN(n))) &&
    Number.isFinite(n)
  );
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Number((x1 * 10 + x2 * 10 + x3 * 10) / 10);
}
